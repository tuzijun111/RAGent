import { c as create_ssr_component, v as validate_component, a as createEventDispatcher, o as onDestroy, b as add_attribute, d as add_styles, e as escape } from './ssr-1WRCSaMf.js';
import ImagePreview from './ImagePreview-Cr7ONVjn.js';
import { B as Block, S as Static, U as UploadText, h as Empty, g as Image, d as StreamingBar, f as BlockLabel, i as IconButtonWrapper, F as FullscreenButton, j as IconButton, H as Clear, w as Image$1, a7 as SelectSource, ah as Webcam$2, a8 as Spinner, ai as Square, aj as Circle, ak as Camera, al as DropdownArrow, a9 as prepare_files } from './2-CNt3iU8Z.js';
import { U as Upload } from './ModifyUpload-7REdIEqm.js';
export { default as BaseExample } from './Example14-L1sJFZD_.js';
...
const Webcam = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  ].join(" ").trim()}"> ${!webcam_accessed ? `<div title="grant webcam access" style="height: 100%">${validate_component(WebcamPermissions, "WebcamPermissions").$$render($$result, {}, {}, {})}</div>` : `<div class="button-wrap svelte-s8feoe"><button${add_attribute("aria-label", mode === "image" ? "capture photo" : "start recording", 0)} class="svelte-s8feoe">${mode === "video" || streaming ? `${streaming && stream_state === "waiting" ? `<div class="icon-with-text svelte-s8feoe" style="width:var(--size-24);"><div class="icon color-primary svelte-s8feoe" title="spinner">${validate_component(Spinner, "Spinner").$$render($$result, {}, {}, {})}</div> ${escape(i18n("audio.waiting"))}</div>` : `${streaming && stream_state === "open" || !streaming && recording ? `<div class="icon-with-text svelte-s8feoe"><div class="icon color-primary svelte-s8feoe" title="stop recording">${validate_component(Square, "Square").$$render($$result, {}, {}, {})}</div> ${escape(i18n("audio.stop"))}</div>` : `<div class="icon-with-text svelte-s8feoe"><div class="icon color-primary svelte-s8feoe" title="start recording">${validate_component(Circle, "Circle").$$render($$result, {}, {}, {})}</div> ${escape(i18n("audio.record"))}</div>`}`}` : `<div class="icon svelte-s8feoe" title="capture photo">${validate_component(Camera, "Camera").$$render($$result, {}, {}, {})}</div>`}</button> ${!recording ? `<button class="icon svelte-s8feoe" aria-label="select input source">${validate_component(DropdownArrow, "DropdownArrow").$$render($$result, {}, {}, {})}</button>` : ``}</div> ${``}`} </div>`;
});
...
const ImageUploader = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `${validate_component(BlockLabel, "BlockLabel").$$render(
      ...
      {},
      {}
    )} <div data-testid="image" class="image-container svelte-1ti4ehe"${add_attribute("this", image_container, 0)}>${validate_component(IconButtonWrapper, "IconButtonWrapper").$$render($$result, {}, {}, {
      default: () => {
        return `${value?.url && !active_streaming ? `${show_fullscreen_button ? `${validate_component(FullscreenButton, "FullscreenButton").$$render($$result, { container: image_container }, {}, {})}` : ``} ${validate_component(IconButton, "IconButton").$$render($$result, { Icon: Clear, label: "Remove Image" }, {}, {})}` : ``}`;
      }
    })} <div class="${[
...
//# sourceMappingURL=Index23-eOgiv_PC.js.map
